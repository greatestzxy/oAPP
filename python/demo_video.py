#python demo_video.py - -test_data_path=test.mp4 - -expected="E4" - -name = "profA"
...
def main(argv=None):
	...
	shown_time_threshold = threshold / 3.5
			...
			if (over ==0):
				...
				if timer2 == 1 :
					...
					if  (hands_detected == 1)&(start_counting==1):
						...
						if mouth_close==1:
							cv2.putText(im[:, :, ::-1], "Starting the 10 second countdown...", (font_location1, font_location2+200), cv2.FONT_HERSHEY_SIMPLEX, font_size, font_color, font_thickness)
							timing = timing + 1
							start_counting = 1
				...
				if timing > threshold:
					cv2.putText(im[:, :, ::-1], "Please open your mouth and show that the pill is still on your tongue.", (font_location1, font_location2+225), cv2.FONT_HERSHEY_SIMPLEX, font_size,
					            font_color, font_thickness)
					time_up = 1
				...
				if time_up == 1:
					if (mouth_close == 0)&(pill_inside==1):
						cv2.putText(im[:, :, ::-1], "Thank you. You accomplished all the steps.", (font_location1, font_location2+25),cv2.FONT_HERSHEY_SIMPLEX, font_size,font_color, font_thickness)
						cv2.putText(im[:, :, ::-1],
						            "In two minutes we will verify that the number was correct.",
						            (font_location1, font_location2 + 50), cv2.FONT_HERSHEY_SIMPLEX, font_size,
						            font_color, font_thickness)
						global finished
						finished = 1
						over = 1
				...
				cv2.putText(im[:, :, ::-1], "Thank you. You accomplished all the steps.",
				            (font_location1, font_location2 + 25), cv2.FONT_HERSHEY_SIMPLEX, font_size, font_color,
				            font_thickness)
				cv2.putText(im[:, :, ::-1],
				            "In two minutes we will verify that the number was correct.",
				            (font_location1, font_location2 + 50), cv2.FONT_HERSHEY_SIMPLEX, font_size,
